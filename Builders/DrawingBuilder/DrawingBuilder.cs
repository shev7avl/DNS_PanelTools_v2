...
namespace DSKPrim.PanelTools.Builders
{
    public class DrawingBuilder
    {
        ...
        public void BuildSheets(Document document)
        {
            ...
            List<Sheet> sheets = Utility.SheetUtils.CreateSheetList(document, elementId, BuilderTemplate.GetNumberOfSheets());
        ...
        public void BuildDrawings(Document document)
        {
            ...
            foreach (var sheet in BuilderTemplate.GetDrawingSetMapping().Keys)
            {
                ...
                {
                    Utility.SheetUtils.PlaceViewsOnSheet(document, sheet, viewRef);
                }
            }
            transaction.Commit();
        }
        ...
        public void BuildParameters(Document document)
        {
            ...
            {
                Utility.SheetUtils.SetSheetParameters(document, elementId, BuilderTemplate.Sheets[i].SheetLink, i + 1);
            }
            transaction.Commit();
        }
        ...
        internal static class AssemblyViewLogic
        {
            ...
            {
                List<AssemblyDetailViewOrientation> orientations = Utility.Assemblies.DefineViewOrientations(document, Panel.AssemblyInstance.Id);
                if (TemplateExists(viewReference))
                {
                    ...
                    else if (viewReference.ViewTemplate.ToString().Contains("SECTION"))
                    {
                        if (SideSection)
                        {
                            ...
                            Utility.SheetUtils.RotatePlanView(document, output, orientations[0]);
                            return output;
                        }
                        else
                        {
