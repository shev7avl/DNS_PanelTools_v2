    ...
    public class ViewsEnvironment: IResettable
    {
        ...
        private Element CheckIfTemplateExists(ViewTemplateName viewTemplateName)
        {
            ...
            if (tempNameString != null)
            {
                ...
                if (viewTemplateName.ToString().Contains("FRONT"))
                {
                    return new FilteredElementCollector(Document).
                        
                        OfClass(viewType).
                        FirstOrDefault(o => o.Name.Contains(tempNameString) && !o.Name.Contains("Сечение"));
                }
                else
                {
                return new FilteredElementCollector(Document).
                        ...
                        OfClass(viewType).
                        FirstOrDefault(o => o.Name.Contains(tempNameString));
            }
            }
            else
            {
        ...
        private readonly Dictionary<ViewTemplateName, string> ViewTemplateNamesEnumMapping = new Dictionary<ViewTemplateName, string>
        {
            ...
            {ViewTemplateName.LEGEND_SLAB_ANNOTATION, "Примечание_Пустотные плиты" },
